...
### Twenty Source of Truth

Before making code changes related to Twenty objects, fields, views, layouts,
front components, GraphQL APIs, or native UI behavior, check the current local
Twenty sources and relevant docs instead of relying on older copied code or
previous implementation notes.

Prefer native Twenty components, state patterns, metadata helpers, and API
shapes whenever an appropriate primitive exists. If no suitable Twenty
component or helper exists, raise the gap before introducing a custom
alternative. Record meaningful source or docs assumptions in implementation
notes or final responses.

For this full Twenty monorepo, use the native Nx/Yarn workflow and
`packages/twenty-utils/setup-dev-env.sh`. The Mark8 app-package workflow is only
