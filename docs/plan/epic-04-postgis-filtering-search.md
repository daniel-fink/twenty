...
Implemented on the private Epic 04 branch and cleaned for upstream review in
`upstream/native-map-view-pr-04-hardening`. The current implementation includes
view-scoped MVT tiles, TileJSON metadata, map tile policy handling, geometry
map sources, benchmark commands, and spatial filter operands named
`WITHIN_DISTANCE`, `WITHIN_BBOX`, `INTERSECTS`, `CONTAINS_GEOMETRY`,
`WITHIN_GEOMETRY`, and `NEAR`.
