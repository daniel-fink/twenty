...
Implemented on the private Epic 02 branch and represented for upstream review
by `upstream/native-map-view-pr-02-map-mvp`. Later epics expanded map views to
support native geometry sources and vector tiles, but this document remains the
address-backed MVP baseline.
...
- Add `record-map` module:
  ...
  - `RecordIndexMapDataLoaderEffect`
...
The current clean upstream branch combines the MVP into one review branch:

1. `upstream/native-map-view-pr-02-map-mvp`: shared/backend map metadata,
   frontend map view rendering, address-backed markers, tests, and fixture data.
